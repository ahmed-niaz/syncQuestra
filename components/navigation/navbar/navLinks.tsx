      ...
      {SideBarLink.map((item) => {
        ...
        const LinkComponent = (
          <Link
            ...
            key={item.label}
            className={cn(
              isActive ? "primary-gradient rounded-lg text-black" : "text-dark300_light900",
              "flex items-center justify-start gap-4 bg-transparent p-4"
            )}
          >
            <Image src={item.imgUrl} alt={item.label} width={20} height={20} className={cn(isActive ? "invert" : "")} />
